...
Подключитесь к серверу под пользователем с `sudo` и выполните **одну
строку** — она скачает и запустит свежий bootstrap-скрипт прямо из
GitHub:

```bash
ssh <ваш-логин>@94.242.58.24
...
Скрипт идемпотентный, выполняется ~2–5 минут на чистой машине, в конце
выведет блок:
...
Если 80-й порт уже занят (например, его слушает сервис Streamlit
на 8501 — это другой порт, не путайте), скрипт всё равно сработает:
он отключит `/etc/nginx/sites-enabled/default` и поставит наш сайт как
`default_server` на 80. Если на 80 действительно стоит другой
обязательный сервис — см. секцию **«Альтернатива: на отдельном порту»**.

## 2. Обновление после изменений в коде

После каждого `git push` в ветку `main` достаточно повторить
ту же команду — bootstrap-скрипт сам подтянет новый код, пересоберёт
фронт и перезапустит бэкенд:
...
```bash
sudo -u deploy bash /home/deploy/Delores-object/scripts/deploy/publish-from-git.sh
```
